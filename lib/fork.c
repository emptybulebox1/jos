...
duppage(envid_t envid, unsigned pn)
{
	...
	if ((pte & PTE_W) || (pte & PTE_COW)) { //If the page is writable or copy-on-write
		r = sys_page_map(0, pg, envid, pg, PTE_P | PTE_U | PTE_COW);
		if (r < 0) return r;
		r = sys_page_map(0, pg, 0, pg, PTE_P | PTE_U | PTE_COW);
		if (r < 0) return r;
	}
	else {
		r = sys_page_map(0, pg, envid, pg, PTE_P | PTE_U);
		if (r < 0) return r;
	}
	return 0;
